/**
 * TypeScript harness generator.
 *
 * Mirrors the JavaScript harness, with one critical difference: Piston's
 * typescript runtime compiles with a bare `tsc *.ts` (no tsconfig, ES5
 * target/lib) before running the emitted JS with node. Under the ES5 lib,
 * everyday code like `new Map()` or `arr.includes(x)` is a *type* error even
 * though it runs fine on node — so the file leads with `// @ts-nocheck` to
 * keep tsc in transpile-only mode. Syntax errors still fail the compile
 * stage; type annotations are documentation, not enforcement (locally the
 * code runs via node's type stripping, which never type-checks either).
 */

import type { FunctionSignature, TestCase } from "../types";

export function buildTypeScriptHarness(
  userCode: string,
  ...
  return `// @ts-nocheck — judge harness; Piston's bare tsc uses the ES5 lib
${userCode}
